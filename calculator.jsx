...
export default function MerlinANACalculator() {
  ...
  const results = useMemo(() => {
    ...
    const categoryResults = Object.keys(categories).map(cat => {
    ...
    const totalSavings = categoryResults.reduce((sum, c) => sum + c.savings, 0);
    const totalAddressable = categoryResults.reduce((sum, c) => sum + c.addressable, 0);
  ...
  const pieData = results.categoryResults.map((c, i) => ({
    name: c.name,
    value: c.savings,
    color: COLORS[i % COLORS.length]
  })).filter(d => d.value > 0);

  const barData = results.categoryResults.map(c => ({
    name: c.name.split(' ')[0],
  ...
  const radarData = results.categoryResults.map(c => ({
        ...
        <section style={{
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <input
                  ...
                  max="30"
                  value={tailSpendPct}
                  onChange={(e) => setTailSpendPct(Number(e.target.value))}
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <input
                  ...
                  onChange={(e) => setCurrentSavingsRate(Number(e.target.value))}
          ...
          <div style={{ marginTop: '32px', paddingTop: '24px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            {useCustomCategories && (
              <p style={{
                fontSize: '13px',
                color: Object.values(categories).reduce((a, b) => a + b, 0) === 100 ? '#02c39a' : '#ff6b6b',
                marginTop: '12px',
                display: 'flex',
                alignItems: 'center',
                gap: '6px'
              }}>
                {Object.values(categories).reduce((a, b) => a + b, 0) === 100 ? (
                  <><CheckCircle2 size={16} /> Categories sum to 100%</>
                ) : (
                  <><AlertCircle size={16} /> Categories sum to {Object.values(categories).reduce((a, b) => a + b, 0)}% (should be 100%)</>
                )}
              </p>
            )}
          </div>
              ...
              <div style={{
                ...
                <ResponsiveContainer width="100%" height={280}>
                  <PieChart>
                    ...
                    />
                    <Legend
                      formatter={(value) => <span style={{ color: '#a8b9cc', fontSize: '12px' }}>{value}</span>}
                    />
                  </PieChart>
                </ResponsiveContainer>
              </div>
